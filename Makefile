...
key: testpub name repo continue replacename replacerepo builddocker rundocker resetname resetrepo rm rm-name
...
rundocker:
	@docker run --name=dockeransibleicinga2 \
	-v ~/.ssh:/tmp/.ssh \
	--cidfile="cid" \
	-v ~/.gitconfig:/root/.gitconfig \
	-v /var/run/docker.sock:/run/docker.sock \
	-v $(shell which docker):/bin/docker \
	-t dockeransibleicinga2

builddocker:
	/usr/bin/time -v docker build -t dockeransibleicinga2 .
